...
Return:      int
**************************************/
int generate_temp(string btn_r,int holes,int position){
    ...
    cout<<top_y<<";"<<left_x<<";"<<center.x<<","<<center.y<<"-"<<pixs_mm<<endl;
...
//    int margin = rect_top.height/10;
    ...
    for (index; index <= total; index++){
        ...
        if (!srcImage.empty()){
            ...
            Mat matchImage ;
            //存入容器中
            matchImage = blurModelDstMat(rect_temps[index-1]).clone();
            modelImages.push_back(matchImage);
            //写入到记录错误测试结果的对应纽扣模板文件中
            imwrite((btn_subdir_model+'/'+to_string(index)+".jpg").c_str(),srcImage);//原图
            rectangle(blurModelDstMat,rect_temps[index-1],Scalar(0,0,255),2,1,0);
            imwrite((btn_subdir_model+'/big_'+to_string(index)+".jpg").c_str(),blurModelDstMat);//矩形框灰度图
            imwrite((btn_subdir_model+'/rect_'+to_string(index)+".jpg").c_str(),matchImage);//模板图
...
int match(string btn_r,int holes,int position){
    Mat frame;
...
//            if (angle==270){angle=90;direction = 0;}
            ...
            if(key=='N' || key=='n')//回车确定误检 收集数据 N=>78
            {
                time_t t = time(0);
                sprintf(filename, (btn_subdir+"n_org_"+to_string(angle)+'_'+to_string(t)+".jpg").c_str());
                cout<<filename<<endl;
                imwrite(filename, frame);

                rectangle(blurMatchDstMat,rects[0],Scalar(0,0,255),2,1,0);
                rectangle(blurMatchDstMat,rects[1],Scalar(0,0,255),2,1,0);
                if(holes==4) {
                    rectangle(blurMatchDstMat,rects[2],Scalar(0,0,255),2,1,0);
                    rectangle(blurMatchDstMat,rects[3],Scalar(0,0,255),2,1,0);
                }
                sprintf(filename, (btn_subdir+"n_rect_"+to_string(angle)+'_'+to_string(t)+".jpg").c_str());
                cout<<filename;
                imwrite(filename, blurMatchDstMat);
                cout<<"保存成功"<<endl;
            }
            if(key=='M' || key=='m') //M键 正确
            {
                time_t t = time(0);
                sprintf(filename, (btn_subdir+"t_"+to_string(angle)+'_'+to_string(t)+".jpg").c_str());
                cout<<filename<<endl;
                imwrite(filename, frame);
                cout<<"保存成功"<<endl;
            }
        ...
        else if(key=='q' || key=='Q'){destroyAllWindows();return 0;}//退出识别
    }
}
...
int main()
{
    ...
    while(1){
        ...
        cin>>select;
        switch (select){
            case 1:cout<<"\n请输入纽扣半径,扣眼数和字符位置（0123）:";cin>>radius>>holes>>position; generate_temp(radius, holes,position);break;
            case 2:cout<<"\n请输入纽扣半径,扣眼数和字符位置（0123）:";cin>>radius>>holes>>position; match(radius,holes,position);break;
            default:return 0;
        }
    }
}
