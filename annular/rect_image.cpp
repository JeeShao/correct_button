...
Mat pertImage(IplImage* srcImage){
    ...
    srcMat = whiteBalance(srcMat);
//    srcMat = illumination(srcMat);
    Mat grayImage;
    Mat blurImage;
    unevenLightCompensate0(srcMat,30);
    if(srcMat.channels()==3)
        cvtColor(srcMat, grayImage, CV_BGR2GRAY);
    else
        grayImage=srcMat.clone();

//    normalize(grayImage, grayImage, 0,1, cv::NORM_L2);
